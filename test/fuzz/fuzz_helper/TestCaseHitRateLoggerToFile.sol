...
error InvalidExportLogMapError(string message, string[] keys, Triple.ParameterStorage[] values, uint256);
...
contract TestCaseHitRateLoggerToFile is PRBTest, StdCheats {
  ...
  function convertHitRatesToString(
    string[] memory keys,
    Triple.ParameterStorage[] memory values
    ...
    if (keys.length > 1) {
      for (uint256 i = 0; i < keys.length - 1; i++) {
        tupleStrings[i] = string(
          abi.encodePacked(
            '{ "number":',
            Strings.toString(values[i].hitCount),
            ', "str": "',
